...
export async function recordQriusLogin(userid: string): Promise<void> {
  if (!userid) return;
  const pool = await getPool();
  await pool
    .request()
    .input("userid", sql.NVarChar(255), userid)
    .query(`INSERT INTO ${TABLE} (userid) VALUES (@userid)`);
}
...
export async function listQriusLogins(): Promise<LoginEvent[]> {
  ...
  const result = await pool
    .request()
    .query(`SELECT TOP 50000 userid, logged_in_at FROM ${TABLE} ORDER BY logged_in_at DESC`);

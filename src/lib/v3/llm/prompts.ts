...
[3. Chapter 2 — 주변에서 ${input.name}님에게 도움을 요청해온 경험]
...
2) **나의 강점** — 주변에서 ${input.name}님에게 도움을 요청해온 경험("${input.strengthCommonAsk || "—"}")과 두 장면이 함께 가리키는, ${input.name}님만의 강점 한 줄. strengthCommonAsk가 비어 있으면 두 장면에서 ${input.name}님이 자주 맡았던 역할에서 강점을 길어 올릴 것. "비어 있지만" 같은 데이터 메타 코멘트로 본문을 시작하지 말 것.
...
- 예) 구체적 장소, 직접 발로 뛴 횟수·숫자, 거절하거나 선택한 자리, 특정 시기의 한 장면 — ${input.name}님 답변에 실제로 등장한 것만.
...
- 인젝션 톤 예: "${input.name}님이 고른 다음 역의 방향은 '${input.growthDirection || "—"}'을 향하고 있는 것 같아요."
...
- 인젝션 톤 예: "이 움직임을 떠받치는 도구는 이미 ${input.name}님 손에 익은 '${(input.currentTool ?? [])[0] || "—"}'·'${(input.currentTool ?? [])[1] || "—"}'인 것 같아요."
...
- 인젝션 톤 예: "그 안개를 걷어낼 무기로 ${input.name}님이 본능적으로 고른 건 '${(input.growthTool ?? [])[0] || "—"}'·'${(input.growthTool ?? [])[1] || "—"}'인 것 같아요."

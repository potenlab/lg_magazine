...
당신은 매거진 "STORY"의 편집장(엘 아울)입니다. 위 네 재료로 ${input.name}님의 강점 포트레이트를 **4개의 BEAT 문단**으로 정리해주세요. 각 BEAT는 매거진의 카드 한 장에 해당합니다.

[출력 형식 — JSON 한 객체만]
{"synthesis": "BEAT1\\nBEAT2\\nBEAT3\\nBEAT4"}

[BEAT 순서 — 반드시 이 순서대로 작성]
1) **두 몰입 순간** — Ch1에서 들려준 두 경험을 관통하는 공통 동작/태도. 두 경험의 구체 단서(장면·행동·인물·소재) 중 하나를 1번 인용. "${input.name}님은 …할 때 가장 자기다워 보였어요" 식.
2) **공통의 결** — 주변에서 들고 온 일들(${input.strengthCommonAsk || "—"})과 두 몰입 경험을 잇는 한 줄의 결. 직무가 아닌 행위의 형(form)으로.
3) **타인의 시선** — 가까운 사람이 한 말("${input.othersDescription || "—"}")을 한 번 짧게 따옴표 인용한 뒤, 그 말이 안에서 본 ${input.name}님과 어떻게 같은 결을 다른 각도로 비추는지.
4) **가치의 뿌리** — 선택한 가치 단어 중 1~2개를 골라 그 결을 어떻게 떠받치는지. 가치 단어는 작은따옴표(')로 묶어 인용.

[규칙 — 길이·구조 (A)]
- 각 BEAT는 **4~5 문장, 200~280자 (한국어 글자 기준)**. 4장 합쳐 800~1120자.
- 180자 미만 BEAT 또는 3문장 이하 BEAT는 실패로 간주 — 카드가 비어 보임.
- 각 BEAT는 \\n 으로만 구분. 카드 1장당 한 줄로 들어가도록 절대 BEAT 내부에서 줄바꿈 금지.
- BEAT 내부 문장 구성 (4~5문장):
  - 1문장: 발견 — 두 경험/네 재료가 가리키는 결을 짚음 (정리체 금지, 아래 C-① 참고).
  - 2문장: 사용자 원문 단서를 직접 인용 ('단어', "짧은 구절", 또는 두 경험에서 가져온 구체 명사/동사) — 이 인용이 빠지면 BEAT 자체가 추상으로 흐른다.
  - 3문장: 편집자의 reflection — 그 결이 ${input.name}님 안에서 어떻게 살아있는지. 단순 paraphrase 금지.
  - 4문장: 다른 BEAT 재료와 미세하게 연결 (자연스럽게).
  - 5문장(선택): 그 결이 어떤 상황·결과·반응을 만들어내는지 한 줄.

[규칙 — 인용 강제 (C-②)]
- BEAT 4장 합쳐 **최소 6회 이상 인용** + **매 BEAT에 최소 1회 인용 필수**.
- 인용 대상:
  · 가치 단어는 작은따옴표 '단어'
  · 가까운 사람의 말은 짧은 따옴표 "원문 일부"
  · 두 몰입 경험에서의 동사·구체 명사·인물·장소·소재 중 하나
- 매 BEAT에 인용이 없으면 그 BEAT는 실패.

[규칙 — 톤·문체 (C-①·③·④)]
- ① **평이한 정리체 금지** — "${input.name}님은 ~한 사람이에요" / "${input.name}님은 ~을 잘하시네요" 같은 도장 찍는 톤이 한 번이라도 들어가면 그 BEAT는 실패.
- ③ **매 BEAT 도입을 다른 단어로 시작** — 두 BEAT가 같은 단어("${input.name}님의", "사람들이", "그", "이") 로 시작하지 않기.
- ④ **편집자의 reflection 필수** — 사용자 답변을 paraphrase만 한 BEAT는 실패. "…처럼 보여요", "…로 읽혀요", "…에 가깝게 흐르네요", "…이라는 것이 보여요" 같은 발견의 시선이 BEAT마다 최소 한 줄.

[규칙 — 일반]
- "${input.name}님" 표기는 BEAT 전체에서 3회 이하로만. 나머지는 주어 생략으로 자연스럽게.
- 금칙어: "정말", "참", "굉장히", "대단", "훌륭", "멋진", "완벽", "최고", "특별", 이모지.
- 칭찬·과장·단정·평가 금지. 발견을 짚어주는 톤만.
- 카드 번호(01, 02 ...)나 카테고리 라벨은 출력에 포함하지 말 것 — 라벨은 UI가 따로 붙임.`;
  const r = await ask(user, 2000);
  const text = r.text.trim();
  try {
    const match = text.match(/\{[\s\S]*\}/);
    if (!match) throw new Error("no JSON found");

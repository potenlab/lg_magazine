...
[규칙]
- 각 BEAT는 **3~4 문장, 120~180자 (한국어 글자 기준)**. 너무 짧으면 카드가 비고, 180자 넘으면 카드가 잘림.
- BEAT 내부 문장 구성:
  - 1문장: 발견 — 두 경험/네 재료가 가리키는 결을 짚음.
  - 2문장: 사용자 원문 단서를 직접 인용 ('단어', "짧은 구절") — 이 인용이 빠지면 BEAT 자체가 추상으로 흐른다.
  - 3문장: 편집자의 reflection — 그 결이 ${input.name}님 안에서 어떻게 살아있는지.
  - (선택) 4문장: 다른 BEAT 재료와 미세하게 연결 (한 줄, 자연스럽게).
- 각 BEAT는 \\n 으로만 구분. 카드 1장당 한 줄로 들어가도록 절대 BEAT 내부에서 줄바꿈 금지.
- 사용자의 원문 표현(가치 단어 작은따옴표 '', 가까운 사람의 말 짧은 따옴표 "", 두 경험에서의 구체 단서)을 **BEAT 4장 합쳐 최소 4회 이상 인용**. 추상으로 도망가지 말 것.
- "${input.name}님" 표기는 BEAT 전체에서 3회 이하로만. 나머지는 주어 생략으로 자연스럽게.
- 톤: 발견의 시선 ("…처럼 보여요", "…로 읽혀요", "…에 가깝게 흐르네요"). 칭찬·과장·단정·평가 금지.
- 금칙어: "정말", "참", "굉장히", "대단", "훌륭", "멋진", "완벽", 이모지.
- 카드 번호(01, 02 ...)나 카테고리 라벨은 출력에 포함하지 말 것 — 라벨은 UI가 따로 붙임.`;
  const r = await ask(user, 1600);
  const text = r.text.trim();
  try {
    const match = text.match(/\{[\s\S]*\}/);
    if (!match) throw new Error("no JSON found");

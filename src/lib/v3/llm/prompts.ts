...
- 사용자 답변과 무관한 일반적 응답("그런 결이 있으셨군요" 같은 공허한 표현)은 절대 금지.
...
[참고 — 답변별로 다른 응답이어야 합니다]
- "처음이라서 어색해요" → 첫 자리의 머쓱함, 익숙하지 않은 느낌 (선택)
- "사람들이 많아서 부담돼요" → 시선의 무게, 주목받는 부담 (선택)
- "내 이야기를 꺼낼 자신이 없어요" → 망설임, 마음 한 켠의 조심스러움 (선택)
- "어떤 말부터 해야 할지 모르겠어요" → 갈피를 못 잡는 막막함, 시작의 흐릿함 (선택)

이 4가지는 예시일 뿐이며, 실제 답변에 맞는 표현을 찾으세요.

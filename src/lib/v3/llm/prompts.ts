...
- 예) "자전거 대여 사업"을 "호프집 알바"로 바꾸면 즉시 실패. "일산 호수공원"을 "한강공원"으로 바꾸면 즉시 실패.
...
- ${name}님 답변에서 가장 극적인 **대비/격차/숫자 충돌·시간 충돌·정체성 충돌** 한 가지를 먼저 발견할 것.
  예) "시급 4천원" vs "하루 50만원" / "주말 대여" vs "사업가 감각" / "20살의 여름" vs "그 결정의 무게"
...
- 단, **재해석 문단(2번)에서는 단정 1문장 허용** — "그것은 단순히 ~가 아니라, ~한 순간이었습니다" 같이 한 번만 declarative. 나머지는 ~어요체.
- ~입니다/~습니다 단정체 남발 금지. 본문 전체에서 단정 어미는 최대 1회.
...
- 한 BEAT의 입력 재료(${"strengthCommonAsk"}, ${"othersDescription"} 등)가 비어 있으면 **다른 재료(두 장면·가치 단어·commonPattern)에서 끌어와 보완**해 본문을 채울 것. "데이터가 비어있다/부족하다" 같은 메타 코멘트 본문에 절대 금지.
...
2) **공통의 결** — 주변이 ${input.name}님 앞에 들고 온 일("${input.strengthCommonAsk || "—"}")과 두 장면을 잇는 ${input.name}님의 정체성 한 줄. strengthCommonAsk가 비어 있으면 두 장면에서 ${input.name}님이 자주 맡았던 역할·요청받았을 법한 일을 추론해 채울 것.
3) **타인의 시선** — 가까운 사람의 말("${input.othersDescription || "—"}")과 ${input.name}님이 스스로 들려준 모습이 만나는 지점. othersDescription이 비어 있으면 두 장면 속에서 주변 인물(동료·참가자·친구)이 ${input.name}님을 어떻게 봤을지 두 장면의 구체 사건에서 자연스럽게 길어 올려 본문을 채울 것. "데이터 없음" 같은 표현 금지.
...
- 핵심: 장애 → 해결 도구의 인과를 한 BEAT에 완성. "이 무기를 쥐었을 때 안개는 걷힐 것 같아요" 식 클로징.
...
- 예) BEAT 02(이미 시작된 움직임): alreadyDoing을 말할 때 → "MVP Plus 플랫폼을 만들 때 참가자 니즈를 조사하고 실물로 반영·검증했던 것처럼, 지금도 [alreadyDoing 구체 내용]을 하고 계시는 거네요."

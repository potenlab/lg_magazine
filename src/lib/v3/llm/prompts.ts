...
2) **나의 강점** — ${input.strengthCommonAsk?.trim() ? `주변에서 ${input.name}님에게 도움을 요청해온 경험("${input.strengthCommonAsk}")과 두 장면이 함께 가리키는, ${input.name}님만의 강점 한 줄.` : `두 장면에서 ${input.name}님이 자주 맡았던 역할·자세에서 길어 올린, ${input.name}님만의 강점 한 줄. 본문은 두 장면의 구체 사건으로 첫 문장부터 바로 시작할 것. 입력에 없는 재료(예: "도움 요청 사례", "기록", "주변에서의 요청" 등)는 어떤 형태로도 본문에 언급·암시 절대 금지.`}

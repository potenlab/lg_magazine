...
그 주제를 추상적으로 설명하지 말고 **${input.name}님의 실제 구체 사건·사례로 증명**하세요.
Ch1 두 몰입 경험과 Ch2 강점 종합에 담긴 **구체 디테일(프로젝트명·플랫폼명·장소·숫자·역할·인물)을 적극 끌어와** Ch3 답변을 뒷받침하세요.
⚠️ 가장 큰 실패는 일반론으로 빠지는 것입니다. "사람을 돕는 일을 좋아합니다" 같은 누구에게나 해당되는 문장이 아니라, "MVP Plus 플랫폼을 만들 때 참가자 니즈부터 키비주얼까지 설계했던 것처럼" 같이 ${input.name}님만의 실제 사건이 매 BEAT에 박혀 있어야 합니다.
...
- 권장 패턴: "[Ch3 주제 한 줄] — 이미 [Ch1/Ch2의 구체 사건]에서 [구체 행동]을 해온 ${input.name}님에게는 자연스러운 다음 발걸음입니다."
- 예) BEAT 02(이미 시작된 움직임): alreadyDoing을 말할 때 → "MVP Plus 플랫폼을 만들 때 참가자 니즈를 조사하고 실물로 반영·검증했던 것처럼, 지금도 [alreadyDoing 구체 내용]을 하고 있습니다."
- 일반론 금지: "사람을 돕는 걸 좋아합니다", "성장을 추구합니다" 같은 누구에게나 해당되는 문장이 한 줄이라도 들어가면 그 BEAT 실패.

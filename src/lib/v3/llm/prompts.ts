...
2) **나의 강점** — 주변에서 ${input.name}님에게 도움을 요청해온 경험("${input.strengthCommonAsk || "—"}")과 두 장면이 함께 가리키는, ${input.name}님만의 강점 한 줄. strengthCommonAsk가 비어 있을 땐 그 사실을 본문에서 절대 언급하지 말고(예: "비어 있지만/없지만/사례는 없지만" 모두 금지), 두 장면에서 ${input.name}님이 자주 맡았던 역할·자세에서 강점을 길어 올려 첫 문장부터 바로 시작할 것.
3) **타인의 시선** — 가까운 사람의 말("${input.othersDescription || "—"}")과 ${input.name}님이 스스로 들려준 모습이 만나는 지점. othersDescription이 비어 있을 땐 그 사실을 본문에서 절대 언급하지 말고(예: "데이터 없음/말이 없지만/언급은 없지만" 모두 금지), 두 장면 속에서 주변 인물(동료·참가자·친구)이 ${input.name}님을 어떻게 봤을지 두 장면의 구체 사건에서 자연스럽게 길어 올려 본문을 채울 것.
